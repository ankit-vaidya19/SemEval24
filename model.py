...
cfg = ConfigDict()
cfg.model_name = "l3cube-pune/hing-bert"
cfg.save_name = "hingbert"
cfg.num_classes = 8
cfg.epochs = 20
...
class SemEvalNet(nn.Module):
    ...
    def get_report(self, labels, predictions):
    ...
    def fit(self, train_loader, val_loader):
        optim = torch.optim.Adam(
            self.parameters(),
            lr=cfg.learning_rate,  # type: ignore
        ...
        for epoch in range(cfg.epochs):
            ...
            train_f1 = self.calculate_f1(train_labels, train_preds)
            self.eval()
            with torch.no_grad():
                ...
                val_f1 = self.calculate_f1(val_labels, val_preds)
            print(f"Training Accuracy - {train_acc}")
            print(f"Training F1 - {train_f1}")
            print(self.get_report(train_labels, train_preds))
            print(f"Val Accuracy - {val_acc}")
            print(f"Val F1 - {val_f1}")
            print(self.get_report(val_labels, val_preds))
            if val_f1 > best_f1:
                best_f1 = val_f1
                print(f"Saved at epoch - {epoch}")
                torch.save(
                    self.state_dict(),
                    f"./{cfg.save_name}-{epoch}.pt",
                )
    ...
    def test(self, test_loader):
        ...
        with torch.no_grad():
            ...
            print(self.get_report(test_labels, test_preds))

    def predict(self, inference_loader, path):
        predictions = []
        self.eval()
        with torch.no_grad():
            for batch in tqdm(inference_loader):
                ...
                predicted_emotions = [
                    emotion_mapping_inv[label] for label in predictions
                ]
